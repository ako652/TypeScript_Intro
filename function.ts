...
function lengthOfName(getName:string): number{
...
function compareNumber(num:number){
 if(num===3){
  return 'equal number'
 }
}
...
function userComparison(users:users[]){
...
// 4. write a function takes 2 parameters : users array and userInput to check if the userInput is matched with the user name
    function checkUserName(users:users[], userInput:string){
      for (let i=0; i<users.length; i++){
